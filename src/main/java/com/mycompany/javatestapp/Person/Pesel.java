...
public class Pesel {
    ...
    public static void PeselValidator(String pesel) {
        if (pesel.length() != 11){
            System.out.println("Podany numer PESEL ma niewłasciwą długość");
            valid = false;
        }
        else {
            for (int i = 0; i < 11; i++){
            ...
            if (checkSum() && checkMonth() && checkDay()) {
                valid = true;
                System.out.println("Podany number PESEL jest właściwy");
            }
            else {
                valid = false;
                System.out.println("Podany numer PESEL jest niewłaściwy");
            }
        }
    }
